...
l_0_pi_y12=zeros(nt,1);
l_pi_2pi_y12=zeros(nt,1);
l_0_pi_y01=zeros(nt,1);
l_pi_2pi_y01=zeros(nt,1);
...
for time=tstart:tstep:tend
	ts=ts+1
	fl=sprintf("lambda_%07d.mat",time)
	ml=matfile(fl);
...
%	l_top=trapz(yCheb(1:110),l(:,1:110),2);
%	l_bot=trapz(yCheb(111:end),l(:,111:end),2);
...
%	d=[];
%	for k=1:Nz/2
%    		botshift=circshift(l_bot,k);
%    		topshift=circshift(l_top,k);
...
%	l=circshift(l,km,1);
	...
	l1=squeeze(mean( l(1:Nz/2,:)    ,1));
        l2=squeeze(mean( l(1+Nz/2:end,:),1));
	%size(yCheb)
	%size(syz1)
	%size(syz2)
	%time
...
%          l_0_pi_y12(ts)=0.25*trapz( flip(yCheb(1:41)) , flip(l1(1:41)));
%        l_pi_2pi_y12(ts)=0.25*trapz( flip(yCheb(1:41)) , flip(l2(1:41)));
%          l_0_pi_y01(ts)=0.25*trapz( flip(yCheb(111:111+40)) , flip(l1(111:111+40)));
%        l_pi_2pi_y01(ts)=0.25*trapz( flip(yCheb(111:111+40)) , flip(l2(111:111+40)));

	  l_0_pi_y12(ts)=0.25*trapz( flip(yCheb(1:110)) , flip(l1(1:110)));
        l_pi_2pi_y12(ts)=0.25*trapz( flip(yCheb(1:110)) , flip(l2(1:110)));
	l_0_pi_y01(ts)=0.25*trapz( flip(yCheb(111:end)) , flip(l1(111:end)));
	l_pi_2pi_y01(ts)=0.25*trapz( flip(yCheb(111:end)) , flip(l2(111:end)));
...
mn=matfile('transferfields_quad_l_mean.mat','Writable',true)
t=[tstart:tstep:tend]';
mn.t=t;
mn.ut=ut;
...
mn.l_0_pi_y12= l_0_pi_y12;
mn.l_pi_2pi_y12= l_pi_2pi_y12;
mn.l_0_pi_y01= l_0_pi_y01;
mn.l_pi_2pi_y01= l_pi_2pi_y01;
%mn.ks=ks;
