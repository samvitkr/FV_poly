...
uQ2=zeros(nz,nx,ny/2);
vQ2=zeros(nz,nx,ny/2);
wQ2=zeros(nz,nx,ny/2);
%fxQ2=zeros(nz,nx,ny);

uQ4=zeros(nz,nx,ny/2);
vQ4=zeros(nz,nx,ny/2);
wQ4=zeros(nz,nx,ny/2);
%fxQ4=zeros(nz,nx,ny);
...
%fn=sprintf('../data/velgrad_lse_ddfilter_j_%03d.mat',jcond);
fn=sprintf('../data/velgrad_lse_uufilter_j_%03d.mat',jcond);
ml=matfile(fn);
%%
u=-0.115;
v=0.065;
...
%fn=sprintf('../data/velgradfield_lse_ddfilter_Q2_j_%03d.mat',jcond);
fn=sprintf('../data/velgradfield_lse_uufilter_Q2_j_%03d.mat',jcond);
...
yp=yCheb+1;
[X,Z,Y]=meshgrid(xp,zp,yp(ny/2+1:end));
%fn=sprintf('../data/velgradfield_lse_ddfilter_Q4_j_%03d.mat',jcond);
fn=sprintf('../data/velgradfield_lse_uufilter_Q4_j_%03d.mat',jcond);
m=matfile(fn,'Writable',true);
m.ucond=u;
m.vcond=v;
m.u=ulse;
