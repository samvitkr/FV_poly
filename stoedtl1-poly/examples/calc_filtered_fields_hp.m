...
fngf=sprintf("velgrad_transfer_fhp_%07d.mat",fileNr)
...
%D=mdf.D;
mgf.dudx= single(ft.transform_to_physical( ( dudxF).*D));
mgf.dvdx= single(ft.transform_to_physical( ( dvdxF).*D));
mgf.dwdx= single(ft.transform_to_physical( ( dwdxF).*D));
