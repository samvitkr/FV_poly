...
%confTensorPhysical= calc_log_conformation( confTensorPhysical );
%det=(confTensorPhysical.Cxx).*( (confTensorPhysical.Cyy).*(confTensorPhysical.Czz)-  (confTensorPhysical.Cyz).^2) +...
%(confTensorPhysical.Cxy).*( (confTensorPhysical.Cyz).*(confTensorPhysical.Cxz) - (confTensorPhysical.Cxy).*(confTensorPhysical.Czz) )+...
%(confTensorPhysical.Cxz).*( (confTensorPhysical.Cxy).*(confTensorPhysical.Cyz) - (confTensorPhysical.Cyy).*(confTensorPhysical.Cxz) );
%vol=sqrt(det);
...
cxzl=confTensorPhysical.Cxz./L_max^2;
cyzl=confTensorPhysical.Cyz./L_max^2;
...
fnt=sprintf("polylength/polylength_%07d.mat",fileNr)
fmt=fullfile(runFolder,fnt)
mt=matfile(fmt,'Writable',true)
%mt.l=   squeeze(mean(stretch,[1,2]));
mt.cxxl=squeeze(mean(cxxl   ,[1,2]));
mt.cyyl=squeeze(mean(cyyl   ,[1,2]));
mt.czzl=squeeze(mean(czzl   ,[1,2]));
mt.cxyl=squeeze(mean(cxyl   ,[1,2]));
mt.cxzl=squeeze(mean(cxzl   ,[1,2]));
mt.cyzl=squeeze(mean(cyzl   ,[1,2]));

%mt.vol=squeeze(mean(vol   ,[1,2]));
%mt.voz=voz;
%mt.woy=woy;
%mt.visc=visc;
%mt.vozF=vozFourier;
